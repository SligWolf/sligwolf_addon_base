AddCSLuaFile()
DEFINE_BASECLASS("sw_weapon_vr_base")
...
function SWEP:CalcVrVelocities()
	...
	for k,v in ipairs(velocityMap) do
		...
		if !posGetterName then
			continue
		end
		
		if !velSetterName then
			continue
		end
...
function SWEP:VRTrace()
	...
	local filterFunc = function(addon, sp, ent)
	...
	for _, traceChain in ipairs(self.VrTraceChains) do
		...
		local tr = Addon:TracerChain(self, tracePoses, filterFunc)
